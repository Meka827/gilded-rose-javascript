describe("update_quality", () => {
  ...
  it("sets quality of backstage passes to 0 when concert has ended", function () {
    const testItem = new BackstagePassItem('Backstage passes to a TAFKAL80ETC concert', -1, 7);
    items.push(testItem);
    update_quality();
    expect(testItem.quality).toEqual(0)
  });  

  it("degrades the quality of 'Conjured' items twice as fast as normal ones", function () {
    const testItem = new ConjuredItem('Conjured Mana Cake', 8, 4);
    items.push(testItem);
    update_quality();
    expect(testItem.quality).toEqual(2)
  });
